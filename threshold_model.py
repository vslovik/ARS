...
class ThresholdModel(object):
    ...
    def __init__(self, graph, threshold=0.5, seed_size=200):
        if not graph.size():
    ...
    def set_seed(self):
        seed = self.get_centers()
        # seed = self.get_center()
...
for i in xrange(len(thresholds)):
    o_sizes.append(ThresholdModel(graph, thresholds[i]).spread())
print(o_sizes)

graph = nx.barabasi_albert_graph(4604, 10)
ba_sizes = []
for i in xrange(len(thresholds)):
    ba_sizes.append(ThresholdModel(graph, thresholds[i]).spread())
print(ba_sizes)

graph = nx.erdos_renyi_graph(4604, 0.005)
er_sizes = []
for i in xrange(len(thresholds)):
    er_sizes.append(ThresholdModel(graph, thresholds[i]).spread())
print(er_sizes)

ThresholdModel.plot_spread_size_distribution(thresholds, o_sizes, ba_sizes, er_sizes)